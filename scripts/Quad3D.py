...
class Quad3D():
    def __init__(self):
        ...
        self.Ku = 10
        self.Tu = 12
        self.Kp = {'x': 2e1, 'y': 2e1, 'z': 1e2,
                   'phi': 1e0, 'theta': 1e0, 'psi': 0.0}
        self.Kd = {'x_dot': 6e0, 'y_dot': 6e0, 'z_dot': 15,
                   'phi_dot': 1e-2, 'theta_dot': 1e-2, 'psi_dot': 0.0}
    ...
    def xdot_3d(self,x,t,xd):
        ...
        return np.squeeze(np.matmul(self.A, x.reshape(12, 1)) + np.matmul(self.B, uu))
    ...
    def simulate(self):
        ...
        B1 = np.zeros((8, 4))
        B2 = np.array([1 / self.m, 0, 0, 0])
        B3 = np.hstack((np.zeros((3, 1)),block_diag([1 / self.Ixx], [1 / self.Iyy], [1 / self.Izz])))
...
def launch_quad3d(ip,tkh,hl,ht):
    ...
    # fly from initial pose to takeoff height
    cp1 = np.array([ip[0],ip[1],ip[2],ip[3], ip[4], ip[5]] + [0]*6)
    dp1 = np.array([ip[0], ip[1],ip[3]+tkh,0, 0, 0] + [0]*6 + [0]*6)
    t0 = 0
    t1 = 2
    ...
    #cp2 =  np.array([ip[0], ip[1], ip[3] + tkh, 0, 0, 0] + [0]*6 + [0]*6)
    ...
    dp3 = np.array([hl[0], hl[1], hl[2], 0, 0, hl[3]] + [0]*6 + [0]*6)
    t3 = ht
    Robot.run_once(cp3, dp3, t1 + t2 + dt, t1+t2+t3)
    ...
    dp4 = np.array([cp4[0], cp4[1], 0, 0, 0, 0, 0, 0, hl[3]] + [0] * 6 + [0] * 6)
    t4 = 1
    Robot.run_once(cp4, dp4, t1+t2+t3 + dt, t1+t2+t3+t4)
